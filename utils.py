...
def to_pixel_samples(img, flatten=True):
...
def to_pixel_samples_split(img):
    """ Convert the image to coord-RGB pairs.
        img: Tensor, (3, H, W)
    """
    H, W = img.shape[-2:]
...
#     x = torch.randn(1,3,256,127)
#     x = (x-x.min())/(x.max()-x.min())
#     y = torch.randn(1,3,256,127)
#     y = (y-y.min())/(y.max()-y.min())
#     ssim_ = calc_ssim(x, y, dataset='benchmark', scale=2)
#     print(ssim_)
